...
function M.check(cb)
  ...
  run({ "gh", "auth", "status" }, function(ok, _, stderr)
    ...
    run({ "gh", "repo", "view", "--json", "nameWithOwner", "-q", ".nameWithOwner" }, function(ok2, out, stderr2)
      if not ok2 then
...
--- Create the release.
--- @param opts table { tag, title, notes, prerelease, draft }
--- @param cb fun(ok: boolean, output: string)
function M.create(opts, cb)
  local cmd = { "gh", "release", "create", opts.tag }
