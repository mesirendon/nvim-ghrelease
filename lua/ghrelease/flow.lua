...
local function body(await)
	...
	while true do
		local intended_tag = await(function(done)
			prompt.buffer_step({ title = "Tag name", default = tag_default }, done)
		end)
		if intended_tag == "" then
			notify("Cancelled: no tag provided.", vim.log.levels.WARN)
			return
		end

		local release_exists, local_tag_exists = await(function(done)
			gh.tag_status(intended_tag, function(re, lte)
				done(re, lte)
			end)
		end)

		tag_exists_locally = local_tag_exists
		...
		local tag_options = {
			{ key = "retry", label = "Retry tag assignation" },
			{ key = "overwrite", label = "Overwrite tag (editing)" },
			{ key = "cancel", label = "Cancel" },
		}

		local tag_option = await(function(done)
			prompt.select(tag_options, {
				prompt = "Tag collision",
				format_item = function(item)
					return item.label
				end,
			}, done)
		...
		if not tag_option or tag_option.key == "cancel" then
			notify("Cancelled.", vim.log.levels.WARN)
			return
		elseif tag_option.key == "overwrite" then
			overwrite = true
			tag = intended_tag
			break
		end
